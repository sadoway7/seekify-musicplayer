...
- Visualizer (iOS, experimental): new attempt to restore audio reactivity on iPhone without breaking AirPlay. Every prior approach (history: b8b88c6, 9051caf, and this session's 077c6ff) tapped a media element via `createMediaElementSource` and connected the analyser to `actx.destination` (the speaker) — that connection claimed the iOS audio session and killed AirPlay. The new variant routes the analyser to a `MediaStreamAudioDestinationNode` instead (a destination that is NOT the speaker): data still flows through the analyser so the visualizer stays reactive, but nothing reaches the speaker output, so iOS may not claim the audio session and AirPlay on the primary element may survive. A hidden secondary `<audio>` plays the same stream for analysis (synced every frame, torn down when the visualizer turns off or during AirPlay). **Untested on device — needs iPhone verification.** If AirPlay still breaks or the analyser gets no data, the fallback is decorative mode. Chrome/Firefox/Android are unchanged (captureStream, zero extra cost). The AirPlay smart-disable is retained: if AirPlay engages, the visualizer hides to album art with a toast ("Visualizer is unavailable during AirPlay") and restores on disconnect.
