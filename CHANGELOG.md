...
- Fix (iOS visualizer): turning the visualizer on no longer breaks AirPlay on Safari/iOS, and turning it off no longer requires a page reload to restore AirPlay. Safari has no `HTMLMediaElement.captureStream()`, so the only frequency-data tap was `createMediaElementSource(Player.audio)` — which irreversibly reroutes the audio element through Web Audio and disables AirPlay for the page lifetime (turning the visualizer off couldn't undo it). Safari/iOS now runs the visualizer in decorative mode (album-derived colors + ambient motion, no frequency input) so the primary element stays on its native output path and AirPlay keeps working. Chrome/Firefox/Android keep full audio reactivity via `captureStream`.
