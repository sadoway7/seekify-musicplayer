...
- Fix (iOS visualizer): the visualizer is now audio-reactive on Safari/iOS AND keeps AirPlay working. Safari has no `HTMLMediaElement.captureStream()`, and tapping the primary element directly (`createMediaElementSource(Player.audio)`) irreversibly reroutes it through Web Audio and disables AirPlay — so the visualizer now plays a SECOND hidden silent `<audio>` element of the same stream and routes only THAT through Web Audio for frequency analysis. The primary element stays on its native output path so AirPlay keeps working; the secondary is muted (gain 0) so it never doubles the audio. The secondary element is synced to the primary every frame (src, play/pause, drift-corrected clock), paused when now-playing hides, and torn down when the visualizer turns off — so the extra fetch only runs while the visualizer is visible. Cost: ~2x bandwidth on the stream while the visualizer is visible (Safari only; Chrome/Firefox/Android use `captureStream` at zero extra cost). If the secondary element can't be set up, Safari falls back to decorative mode (album colors + ambient motion, no pulse).
