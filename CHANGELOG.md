...
- Visualizer (iOS/AirPlay): while audio streams to an AirPlay device, the visualizer now hides itself (back to album art) and the toggle is disabled — a non-reactive decorative visualizer is pointless during a remote session. The user is told why: a toast ("Visualizer is unavailable during AirPlay") appears the moment AirPlay hides the visualizer, and again if they tap the dimmed toggle. When AirPlay disconnects, the visualizer restores if the user had it on. Detection uses Safari's `webkitCurrentPlaybackTargetIsWireless` (and the standard `RemotePlayback` API where available); this is viable now that Safari runs decorative (no Web Audio graph to block AirPlay from engaging).
